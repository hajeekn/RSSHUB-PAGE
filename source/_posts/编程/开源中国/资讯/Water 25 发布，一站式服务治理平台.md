...
<ul> 
 <li>权限系统由旧的bcf框架切换为grit框架</li> 
 <li>/run/、/msg/ 开头的触发规则统一为：/_run/ 开头</li> 
 <li>助理工具 wateraide 更名为：xwater</li> 
 <li>增加 通过令牌安全模式，获取服务运行时状态</li> 
 <li>取消 getDistributionListByMsg 接口缓存，避免造成消息重派</li> 
 <li>修复 行为记录时，没有记录行为者的ip问题（涉及water.client更新）</li> 
 <li>取消 镜像统一端口8080，改为各自不冲突端口</li> 
 <li>升级 solon, snack3, weed3</li> 
 <li>升级 redisx，支持 user 配置（无用的user要去掉了）</li> 
 <li>日志服务的 es 方案，由别名方案改为 data steam 模式</li> 
</ul> 
